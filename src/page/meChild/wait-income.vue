<template>
  <div class="wait-budget">
    <div class="wait-budget-top">
      <m-header :title="title" :is_goBack="is_goBack"></m-header>
      <div class="m-wait-budget-main-top wait-budget_top_select">
        <a @click="changeDate('m')" :class="{'selected':changeDateType=='m'?true:false}">当月待收</a>
        <a @click="changeDate('q')" :class="{'selected':changeDateType=='q'?true:false}">季度待收</a>
        <a @click="changeDate('y')" :class="{'selected':changeDateType=='y'?true:false}">年度待收</a>
      </div>
      <div class="transition">
        ...
        <div class="transition-right">
          总待收金额:
          <br />
          <span class="orange">{{parseFloat(list.total).toFixed(2)}}</span>
        </div>
      </div>
    </div>
    <van-swipe-cell v-for="(item,index) in list.list" :key="index">
      <div class="wait-budget-list" @click="goToRouter(item.type,item.id)">
        <div class="wait-budget-list-left">
          <span v-if="red = item.type==2?true:false"></span>
          <span class="wait-budget-list-left-iconfont iconfont" :class="{'red':red} ">&#xe644;</span>
          <div class="wait-budget-list-left-title">
            <div class="wait-budget-list-left-info">
              <span>{{item.company_name}}</span>
            </div>
            <div
              v-if="item.total_money-item.paid_money"
              :class="{'red':red,'green':!red}"
              class="wait-budget-list-left-info wait-budget-list-main"
            >
              <span>待收款:</span>
              <span>{{item.total_money-item.paid_money}}</span>
            </div>
            <div class="wait-budget-list-left-info">{{item.date}}</div>
          </div>
        </div>
        <div class="wait-budget-list-right-money">
...
export default {
  ...
  methods: {
    ...
    changeToMonth() {
      ...
      this.date_name = "当月";
      console.log(this.screen_start, this.start_date);
      this.getWaitIncome();
    },
    changeToYear() {
      let yy = this.currentDate.getFullYear();
...
.wait-budget {
  ...
  .m-wait-budget-main-top > div {
    flex: 1;
    padding: 5px 15px;
  }
  .m-wait-budget-main-top > div > p > span {
    font-size: 25px;
  }
  .small-font {
    font-size: 14px;
  ...
  .wait-budget-list {
    ...
    position: relative;
    left: 0;
    .wait-budget-list-left {
      display: flex;
      align-items: center;
      .wait-budget-list-left-iconfont {
        color: #57a976;
        font-size: 40px;
